  ...
  return (
    <div className="space-y-5">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline">
            {selectedGenre ? genreMapping[selectedGenre] : "ジャンルを選択"}
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent>
          <DropdownMenuItem
              ...
              event={{
                ...
                user: {
                  ...
                  emailVerified: event.user.emailVerified
                    ? new Date(event.user.emailVerified)
                    : null,
                  ...
                  organizer: event.user.organizer
                    ? {
                        ...
                        createdAt: new Date(event.user.organizer.createdAt),
                        updatedAt: new Date(event.user.organizer.updatedAt),
                      }
                    : null,
                  vendor: event.user.vendor
                    ? {
                        ...event.user.vendor,
                        createdAt: new Date(event.user.vendor.createdAt),
                        updatedAt: new Date(event.user.vendor.updatedAt),
                      }
                    : null,
                  customer: event.user.customer
                    ? {
                        ...event.user.customer,
                        createdAt: new Date(event.user.customer.createdAt),
                        updatedAt: new Date(event.user.customer.updatedAt),
                      }
                    : null,
                },
              }}
      ...
      {events && events.totalEvents && events.events.length > 0 && (
        <PaginationButton
          pageCount={Math.ceil(events.totalEvents / limit)}
          displayPerPage={eventPerPage}
        />
      )}
    </div>
  );
